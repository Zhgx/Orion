...
class ResourceCommands():
    ...
    def setup_commands(self,parser):
        ...
        res_parser = parser.add_parser('resource',
                                        aliases='r',
        ...
        p_create_res.add_argument('resource', metavar='RESOURCE', action='store', help='Name of the resource')
        p_create_res.add_argument('-s', dest='size', action='store',
        ...
        p_create_res.add_argument('-gui', dest='gui', action='store_true', help=argparse.SUPPRESS,
                                          default=False)
        ...
        group_manual_diskless = p_create_res.add_argument_group(title='diskless create')
        ...
        #Add parameter groups for adding resource mirrors
        group_add_mirror = p_create_res.add_argument_group(title='add mirror way')
        ...
        """
        p_delete_res = res_subp.add_parser('delete', aliases='d', help='Delete the resource',
                                                       usage=usage.resource_delete)
        self.p_delete_res = p_delete_res
        p_delete_res.add_argument('resource', metavar='RESOURCE', action='store',
                                          help='Name of the resource to delete')
        p_delete_res.add_argument('-n', dest='node', action='store',
                                          help='The name of the node. In this way, the cluster retains the attribute of the resource, including its name and size.')
        p_delete_res.add_argument('-y', dest='yes', action='store_true', help='Skip to confirm selection',
                                          default=False)
        p_delete_res.add_argument('-gui', dest='gui', action='store_true', help=argparse.SUPPRESS,
                                          default=False)
        ...
        """
        p_show_res = res_subp.add_parser('show', aliases='s', help='Displays the resource view',
                                                     usage=usage.resource_show)
        self.p_show_res = p_show_res
        p_show_res.add_argument('resource', metavar='RESOURCE',
                                        help='Print information about the resource in LINSTOR cluster', action='store',
                                        nargs='?')
        p_show_res.add_argument('--no-color', dest='nocolor', help='Do not use colors in output.',
