...
def get_answer():
    ...
    if rpl == 'no':
        answer = input()
        logger.write_to_log('DATA', 'INPUT', 'confirm_input',
        ...
        def wrapper(cmd):
            ...
            if RPL == 'no':
                ...
                id_result = logdb.get_id(
                    consts.glo_tsc_id(), func_name, consts.glo_log_id())
    ...
    def decorate(func):
        ...
        def wrapper(self, *args):
            ...
            if RPL == 'no':
                ...
                logger.write_to_log(
                    'OPRT', 'JSON', func.__name__, oprt_id, args)
                result = func(self, *args)
                logger.write_to_log(
                    'DATA', 'JSON', func.__name__, oprt_id, result)
            else:
                logdb = consts.glo_db()
                id_result = logdb.get_id(
                    consts.glo_tsc_id(), func.__name__, consts.glo_log_id())
