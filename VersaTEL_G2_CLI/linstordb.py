...
class CollectData(LinstorDB):
    ...
    def _get_resource(self):
        ...
        res_all = self.fet_all(self.select_all(
            ['resourcetb'], 'DISTINCT Resource', 'Allocated', 'DeviceName', 'InUse'))
        in_use = self.fet_all(self.select(
            ['resourcetb'], 'DISTINCT Resource', 'Allocated', 'DeviceName', 'InUse', InUse='InUse'))
    ...
    def get_all_node(self):
        data_list = []
        node_data = self.fet_all(self.select_all(
            ['nodetb'], 'Node', 'NodeType', 'Addresses', 'State'))
        for i in node_data:
    ...
    def get_node_info(self, node):
        n = self.fet_one(self.select(
            ['nodetb'], 'Node', 'NodeType', 'Addresses', 'State', Node=node))
        if n:
    ...
    def get_one_node(self, node):
        date_list = []
        res_data = self.fet_all(self.select(
            ['resourcetb'], 'Resource', 'StoragePool', 'Allocated', 'DeviceName', 'InUse', 'State', Node=node))
    ...
    def get_sp_in_node(self, node):
        data_list = []
        sp_data = self.fet_all(self.select_all(['storagepooltb'], 'StoragePool', 'Node',
                                               'Driver', 'PoolName', 'FreeCapacity', 'TotalCapacity', 'SupportsSnapshots', 'State'))
        for i in sp_data:
            sp_name, node_name, driver, pool_name, free_size, total_size, snapshots, status = i
            res_num = self.select_count(
                ['resourcetb'], 'Resource', Node=node_name, StoragePool=sp_name)
            if node_name == node:
                list_one = [
                    sp_name,
                    node_name,
    ...
    def get_all_res(self):
        ...
        for i in self._get_resource():
            if i[1]:  # 过滤size为空的resource
                ...
                mirror_way = self.select_count(
                    ['resourcetb'], 'Resource', Resource=resource)
    ...
    def get_one_res(self, resource):
        data_list = []
        res_data = self.fet_all(self.select(
            ['resourcetb'], 'Node', 'StoragePool', 'InUse', 'State', Resource=resource))
    ...
    def get_sp_info(self, sp):
        ...
        node = self.fet_all(self.select(
            ['storagepooltb'], 'Node', StoragePool=sp))
        if len(node) == 1:
            names = node[0][0]
        else:
    ...
    def get_one_sp(self, sp):
        date_list = []
        res_data = self.select(['resourcetb'], 'Resource', 'Allocated',
                               'DeviceName', 'InUse', 'State', StoragePool=sp)
        for res in res_data:
