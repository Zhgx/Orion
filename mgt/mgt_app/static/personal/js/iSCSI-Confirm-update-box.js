	...
	if (host_name_hid == "1" && host_iqn_hid == "1") {
		...
		$.ajax({
			...
			success : function(operation_feedback_prompt) {
				write_to_log(tid, 'OPRT', 'ROUTE', masterIp, '/host/create' ,operation_feedback_prompt);
			...
			error : function() {
				write_to_log(tid, 'OPRT', 'ROUTE', masterIp, '/host/create', 'error');
			}
		})
...
$("#host_group_create").click(function() {
	...
	if (hg_name_hid == "1") {
		...
		$.ajax({
			...
			success : function(operation_feedback_prompt) {
				write_to_log(tid, 'OPRT', 'ROUTE', masterIp, '/hg/create' ,operation_feedback_prompt);
				alert(operation_feedback_prompt);
				$("#host_group_name").val("");
			...
			error : function() {
				write_to_log(tid, 'OPRT', 'ROUTE', masterIp, '/hg/create', 'error');
...
$("#map_create").click(function() {
	...
	if (map_name_hid == "1") {
		...
		$.ajax({
			...
			success : function(operation_feedback_prompt) {
				write_to_log(tid, 'OPRT', 'ROUTE', masterIp, '/map/create' ,operation_feedback_prompt);
				alert(operation_feedback_prompt);
				$("#map_name").val("");
				$("#map_name_hid").val("0");
			},
		...
		if (!match_result) {
			...
			$
					.ajax({
						...
						error : function() {
							write_to_log(tid, 'OPRT', 'ROUTE', masterIp,'/host/show/oprt', 'error');
						}
					});
		}
	}
				...
				success : function(HG_result) {
					...
					.ajax({
						...
						success : function(HG_result_data) {
							write_to_log(tid,'DATA','ROUTE',masterIp,'/hg/show/data', JSON.stringify(HG_result_data));
						...
						success : function(map_result) {
							...
							$
									.ajax({
										...
										dataType : "json",
										success : function(Map_result) {
											write_to_log(tid,'DATA','ROUTE',masterIp,'/map/show/data',Map_result);
